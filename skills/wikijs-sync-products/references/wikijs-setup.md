...
the worker. That Wiki.js database is separate from both the read-only
PostgreSQL product catalogue and n8n's PostgreSQL database. The installer
neither provisions the Wiki.js database nor writes Wiki.js data directly to
any database; all page
...
final prefix with private/unpublished pages for acceptance. If a distinct
staging prefix is mandatory, give it a separate worker state volume and plan
an explicit migration; a staging success in production state otherwise
delays that product's next run.
...
New pages are private drafts by default. After validating the acceptance output
...
visibility returned by Wiki.js; the worker never changes it implicitly.
...
Wiki.js directories are path-based; the worker does not edit global navigation.
Datasheets are external links and are not uploaded to Wiki.js storage.
