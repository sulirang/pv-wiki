    ...
    *,
    ...
    request = {
        ...
        "paragraph_construction_rules": [
            ...
            "Chinese-number counts are forbidden. grounded_three_phase_topology alone "
            "may use <完整语义 name_zh 核心>为三相, strictly under its exact independent-"
            "clause mode rule. Never infer it from identity or copy 3L+N+PE.",
            "Never calculate or state a new numeric result from multiple rows, even "
            "when arithmetic is exact. This includes newly derived ratios, multiples, "
            "percentages, sums, differences, products, quotients, oversizing factors, "
            "or utilization values, even if equal to a supplied row or disguised as "
            "一半, 翻番, or 倍增. If no single row contains the exact expression, discuss "
            "it only qualitatively without numeric values or synonyms. Never transliterate "
            "a rejected result.",
        ...
        "analysis_policy": [
            ...
            "exact_technical_tokens_only, use listed exact tokens only in source "
            "order and without repetition. For grounded_three_phase_topology, follow "
            "its exact independent-clause rule. For no_numeric_restatement, use qualitative "
            "text without numeric values, synonyms, codes, ratios, multiples, or 倍.",
