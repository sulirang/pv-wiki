...
HERMES_DECISION_SCHEMA_VERSION = "3"
HERMES_COMPATIBLE_DECISION_SCHEMA_VERSIONS = frozenset({"2", "3"})
...
PUBLICATION_EVENT_TABLE = "product_publication_events"
CONTROL_EVENT_TABLE = "product_publication_control_events"
_CREATE_EVENT_TABLES_SQLITE = (
    f"""
    CREATE TABLE IF NOT EXISTS {PUBLICATION_EVENT_TABLE} (
        event_id INTEGER PRIMARY KEY AUTOINCREMENT,
        product_id TEXT NOT NULL REFERENCES {COMPLETION_TABLE}(product_id),
        event_type TEXT NOT NULL
            CHECK (event_type IN ('direct', 'rerender_preview', 'rerender_apply')),
        wiki_path TEXT,
        content_sha256 TEXT,
    ...
    """,
    f"""
    CREATE INDEX IF NOT EXISTS product_publication_events_product_idx
    ON {PUBLICATION_EVENT_TABLE}(product_id, event_id)
    """,
    f"""
    CREATE TABLE IF NOT EXISTS {CONTROL_EVENT_TABLE} (
        event_id INTEGER PRIMARY KEY AUTOINCREMENT,
        product_id TEXT NOT NULL REFERENCES {COMPLETION_TABLE}(product_id),
        action TEXT NOT NULL CHECK (action IN ('suppress', 'resume')),
        reason TEXT NOT NULL,
        created_at TEXT NOT NULL
    )
    """,
    f"""
    CREATE INDEX IF NOT EXISTS product_publication_control_events_product_idx
    ON {CONTROL_EVENT_TABLE}(product_id, event_id)
    """,
)
_CREATE_EVENT_TABLES_POSTGRES = (
    _CREATE_EVENT_TABLES_SQLITE[0].replace(
        "INTEGER PRIMARY KEY AUTOINCREMENT",
        "BIGINT GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY",
    ),
    _CREATE_EVENT_TABLES_SQLITE[1],
    _CREATE_EVENT_TABLES_SQLITE[2].replace(
        "INTEGER PRIMARY KEY AUTOINCREMENT",
        "BIGINT GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY",
    ),
    _CREATE_EVENT_TABLES_SQLITE[3],
)
...
class PublicationFenceError(CompletionError):
    """Raised when publication control changed during a fenced operation."""
...
@dataclass(frozen=True, slots=True)
class RenderModel:
    """Renderer-facing adapter that never rewrites a stored decision."""

    product: dict[str, Any]
    decision: dict[str, Any]

    @classmethod
    def from_completion(cls, completion: ResearchCompletion) -> "RenderModel":
        return cls.from_decision(completion.product, completion.decision)
    ...
    def from_decision(
        ...
        if schema_version == "2":
            return cls(dict(product), dict(decision))
        if schema_version != "3":
            raise CompletionValidationError(
                "stored decision schema is unsupported; refusing to render"
            )
        rendered = dict(decision)
        parameters = _analysis_parameter_rows(
            decision.get("parameters", []),
        )
        enrichment = decision.get("parameter_enrichment")
        if enrichment is not None:
            if not isinstance(enrichment, Mapping):
                raise CompletionValidationError(
                    "stored parameter_enrichment is invalid"
                )
            try:
                rendered["datasheet_parameters"] = (
                    apply_parameter_translations(parameters, enrichment)
                )
                rendered["professional_analysis"] = professional_analysis(
                    enrichment,
                    parameters,
                )
            except ParameterAnalysisError as exc:
                raise CompletionValidationError(
                    "stored parameter enrichment is invalid"
                ) from exc
        else:
            rendered["datasheet_parameters"] = parameters
        category_code = str(decision.get("product_category_code") or "")
        rendered["product_category"] = PRODUCT_CATEGORY_LABELS.get(
            category_code,
            str(decision.get("product_category") or ""),
        )
        return cls(dict(product), rendered)
...
def _normalize_evidence_documents(
    ...
    *,
    ...
    bodies: dict[str, str] = {}
    receipt_v2_by_url: dict[str, dict[str, Any]] = {}
    for index, raw in enumerate(documents):
        ...
        try:
            if isinstance(receipt_value, Mapping):
                receipt_payload = verify_evidence_receipt_v2(
                    evidence_hmac_key,
                    content=content,
                    receipt=receipt_value,
                )
                receipt_url = receipt_payload["final_url"]
            else:
        ...
        if previous is None:
            ...
            if receipt_payload is not None:
                receipt_v2_by_url[url] = receipt_payload
                evidence_record.update(
                    {
                        "receipt_version": receipt_payload["version"],
                        "requested_url": receipt_payload["requested_url"],
                        "artifact_sha256": receipt_payload["artifact_sha256"],
                        "parser_metadata": receipt_payload["parser_metadata"],
                        "redirect_chain": receipt_payload["redirect_chain"],
                    }
                )
            normalized.append(evidence_record)
    normalized.sort(key=lambda item: item["url"].casefold())
    return normalized, bodies, receipt_v2_by_url
...
def _chinese_text(
    ...
    *,
    ...
    text = _text(value, name=name, required=required, maximum=maximum)
    if text and _HAN_TEXT_RE.search(text) is None:
        raise CompletionValidationError(f"{name} must contain Chinese text")
    return text


def _analysis_parameter_rows(value: Any) -> list[dict[str, Any]]:
    rows = _json_array(value, name="parameters")
    if len(rows) > 200:
        raise CompletionValidationError("parameters may contain at most 200 rows")
    normalized: list[dict[str, Any]] = []
    for index, raw in enumerate(rows):
        item = _json_object(raw, name=f"parameters[{index}]")
        expected_id = f"p{index + 1:03d}"
        if item.get("parameter_id") != expected_id:
            raise CompletionValidationError(
                "parameters must use server-assigned p001.. IDs"
            )
        normalized.append(
            {
                "parameter_id": expected_id,
                "name": _text(
                    item.get("source_label"),
                    name=f"parameters[{index}].source_label",
                    required=True,
                    maximum=200,
                ),
                "section": _text(
                    item.get("section"),
                    name=f"parameters[{index}].section",
                    maximum=100,
                ),
                "subsection": "",
...
def _source_decimal(value: Any, name: str) -> Decimal:
    if isinstance(value, bool):
        raise CompletionValidationError(f"{name} must be numeric")
    candidate = (
        str(value)
        if isinstance(value, (int, float))
        else unicodedata.normalize("NFKC", value).strip()
    ...
    if _SOURCE_NUMERIC_RE.fullmatch(candidate) is None:
        raise CompletionValidationError(f"{name} must be numeric")
    try:
        result = Decimal(re.sub(r"[,\u00a0\u202f ]", "", candidate))
    except InvalidOperation as exc:
        raise CompletionValidationError(f"{name} must be numeric") from exc
    if not result.is_finite():
        raise CompletionValidationError(f"{name} must be finite")
    return result


def _recompute_derived_value(
    *,
    formula: str,
    reported_value: Any,
    ...
    if match is None:
        raise CompletionValidationError(
            f"{name}.formula must be two numeric literals and one operator"
        )
    operands = (
        _source_decimal(match.group(1), f"{name}.formula operand 1"),
        _source_decimal(match.group(3), f"{name}.formula operand 2"),
    )
    expected = (
        _source_decimal(basis_values[0], f"{name}.basis[0]"),
        _source_decimal(basis_values[1], f"{name}.basis[1]"),
    )
    if operands != expected:
        raise CompletionValidationError(
            f"{name}.formula operands must match their parameter rows"
        )
    if (
        isinstance(reported_value, bool)
        or not isinstance(reported_value, (int, float))
        or isinstance(reported_value, float)
        and not math.isfinite(reported_value)
    ):
        raise CompletionValidationError(f"{name}.value must be finite")
    ...
    with localcontext() as context:
        ...
        if operator == "+":
            ...
            if operands[1] == 0:
                raise CompletionValidationError(
                    f"{name}.formula cannot divide by zero"
                )
            result = operands[0] / operands[1]
    reported = Decimal(str(reported_value))
    tolerance = max(abs(result) * Decimal("1e-9"), Decimal("1e-12"))
    if abs(reported - result) > tolerance:
        raise CompletionValidationError(
            f"{name}.value does not match deterministic recomputation"
        )
    if result == result.to_integral_value():
        return int(result)
    final = float(result)
    if not math.isfinite(final):
        raise CompletionValidationError(f"{name}.value is out of range")
    return final


def _normalize_derived_insights(
    value: Any,
    parameters: Sequence[Mapping[str, Any]],
) -> list[dict[str, Any]]:
    items = _json_array(value, name="derived_insights")
    if len(items) > 5:
        raise CompletionValidationError(
            "derived_insights may contain at most 5 entries"
        )
    source_by_id = {
        str(item["parameter_id"]): item for item in parameters
    }
    normalized: list[dict[str, Any]] = []
    names: set[str] = set()
    for index, raw in enumerate(items):
        prefix = f"derived_insights[{index}]"
        item = _json_object(raw, name=prefix)
        if set(item) - {
            "name",
            "value",
            "unit",
            ...
            raise CompletionValidationError(f"{prefix} has unknown fields")
        display_name = _chinese_text(
            item.get("name"),
            name=f"{prefix}.name",
            required=True,
            maximum=200,
        ...
        if not name_key or name_key in names:
            raise CompletionValidationError(
                "derived insight names must be unique"
            )
        names.add(name_key)
        basis_ids = item.get("basis_parameter_ids")
        if (
            not isinstance(basis_ids, list)
            or len(basis_ids) != 2
            or any(not isinstance(entry, str) for entry in basis_ids)
            or len(set(basis_ids)) != 2
            or any(entry not in source_by_id for entry in basis_ids)
        ):
            raise CompletionValidationError(
                f"{prefix}.basis_parameter_ids must name two distinct parameters"
            )
        formula = _text(
            item.get("formula"),
            name=f"{prefix}.formula",
            required=True,
            maximum=500,
        )
        basis_rows = [source_by_id[item_id] for item_id in basis_ids]
        result: dict[str, Any] = {
            "name": display_name,
            "value": _recompute_derived_value(
                formula=formula,
            ...
            "formula": formula,
            "basis": [
                str(row["source_label"]) for row in basis_rows
            ],
        ...
        if "explanation" in item:
            result["explanation"] = _chinese_text(
                item.get("explanation"),
                name=f"{prefix}.explanation",
                required=True,
                maximum=500,
            )
        normalized.append(result)
...
def _normalize_v3_extensions(
    decision: Mapping[str, Any],
    *,
    publishable: bool,
    receipt_v2_by_url: Mapping[str, Mapping[str, Any]],
) -> dict[str, Any]:
    category_code = _text(
        decision.get("product_category_code"),
        name="decision.product_category_code",
        required=publishable,
        maximum=50,
    )
    if category_code and category_code not in PRODUCT_CATEGORY_LABELS:
        raise CompletionValidationError(
            "decision.product_category_code is invalid"
        )
    parameters = _json_array(
        decision.get("parameters", []),
        name="decision.parameters",
    )
    signed_sets = [
        payload.get("parameter_rows", [])
        for payload in receipt_v2_by_url.values()
        if payload.get("parameter_rows")
    ]
    if parameters and not any(
        _canonical_json(parameters) == _canonical_json(signed)
        for signed in signed_sets
    ):
        raise CompletionValidationError(
            "decision.parameters must exactly match one signed PDF parameter set"
        )
    if publishable and signed_sets and not parameters:
        raise CompletionValidationError(
            "publish decisions must carry the signed PDF parameter set"
        )
    analysis_rows = _analysis_parameter_rows(parameters)
    parameter_hash = (
        parameter_set_sha256(analysis_rows)
        if analysis_rows
        else hashlib.sha256(b"[]").hexdigest()
    )
    enrichment_value = decision.get("parameter_enrichment")
    normalized_enrichment: dict[str, Any] | None = None
    if enrichment_value is not None:
        if not analysis_rows:
            raise CompletionValidationError(
                "parameter_enrichment requires parameters"
            )
        try:
            normalized_enrichment = validate_parameter_enrichment(
                enrichment_value,
                analysis_rows,
            )
        except ParameterAnalysisError as exc:
            raise CompletionValidationError(
                "parameter_enrichment is invalid or incomplete"
            ) from exc
    return {
        "product_category_code": category_code,
        "product_type": _chinese_text(
            decision.get("product_type"),
            name="decision.product_type",
            required=publishable,
            maximum=100,
        ),
        "product_description_zh": _chinese_text(
            decision.get("product_description_zh"),
            name="decision.product_description_zh",
            required=publishable,
            maximum=500,
        ),
        "manufacturer_zh": _chinese_text(
            decision.get("manufacturer_zh"),
            name="decision.manufacturer_zh",
            required=publishable,
            maximum=200,
        ),
        "parameters": parameters,
        "parameter_set_sha256": parameter_hash,
        "parameter_enrichment": normalized_enrichment,
        "derived_insights": _normalize_derived_insights(
            decision.get("derived_insights", []),
            parameters,
        ),
    }
...
def validate_research_decision(
    ...
    *,
    ...
    schema_version = decision.get("schema_version")
    if schema_version not in HERMES_COMPATIBLE_DECISION_SCHEMA_VERSIONS:
        raise CompletionValidationError(
            "decision.schema_version is unsupported; refusing future schema"
        )
    ...
    if schema_version == "3":
        allowed_decision_fields.update(
            {
                "manufacturer_zh",
    ...
    evidence, bodies, receipt_v2_by_url = _normalize_evidence_documents(
        evidence_documents,
    ...
    raw_category = _text(
        ...
        required=(
            ...
            or schema_version == "2" and publishable
        ),
        maximum=100,
    )
    ...
    if schema_version == "3":
        normalized.update(
            _normalize_v3_extensions(
                decision,
                publishable=publishable,
                receipt_v2_by_url=receipt_v2_by_url,
            )
        )
        category_code = normalized["product_category_code"]
        if category_code:
            normalized["product_category"] = PRODUCT_CATEGORY_LABELS[
                category_code
            ]
...
class HermesCompletionStore:
    ...
    def _initialize(self) -> None:
        with self._state._write_transaction() as connection:
            ...
            event_schema = (
                ...
                else _CREATE_EVENT_TABLES_SQLITE
            )
    ...
    @staticmethod
    def _publication_fence_on_connection(
        connection: Any,
        product_id: str,
    ) -> str:
        publication = connection.execute(
            f"""
            SELECT MAX(event_id) AS latest
            FROM {PUBLICATION_EVENT_TABLE}
            WHERE product_id = ?
            """,
            (product_id,),
        ).fetchone()
        control = connection.execute(
            f"""
            SELECT MAX(event_id) AS latest
            FROM {CONTROL_EVENT_TABLE}
            WHERE product_id = ?
            """,
            (product_id,),
        ).fetchone()
        return (
            f"p{int(publication['latest'] or 0)}:"
            f"c{int(control['latest'] or 0)}"
        )

    def publication_fence(self, product_id: Any) -> str:
        normalized_product_id = str(product_id)
        with self._state._connection() as connection:
            if connection.execute(
                f"SELECT 1 FROM {COMPLETION_TABLE} WHERE product_id = ?",
                (normalized_product_id,),
            ).fetchone() is None:
                raise ProductNotFoundError("completion does not exist")
            return self._publication_fence_on_connection(
                connection,
                normalized_product_id,
            )
    ...
    def _assert_publication_fence(
        ...
        current = HermesCompletionStore._publication_fence_on_connection(
            connection,
            product_id,
        )
        if current != expected_fence:
            raise PublicationFenceError(
                "publication state changed during the fenced operation"
            )
    ...
    def append_control_event(
        ...
        *,
        ...
        if action not in {"suppress", "resume"}:
            raise ValueError("control action must be suppress or resume")
        ...
        with self._state._write_transaction() as connection:
            ...
            connection.execute(
                f"""
                INSERT INTO {CONTROL_EVENT_TABLE} (
                    product_id, action, reason, created_at
                ) VALUES (?, ?, ?, ?)
                """,
                (
                    normalized_product_id,
    ...
    def is_suppressed(self, product_id: Any) -> bool:
        with self._state._connection() as connection:
            row = connection.execute(
                f"""
                SELECT action
        ...
        return row is not None and row["action"] == "suppress"
    ...
    def record_publication_event(
        ...
        *,
        ...
        if event_type not in {
            ...
            raise ValueError("publication event type is invalid")
        normalized_product_id = str(product_id)
        with self._state._write_transaction() as connection:
            if connection.execute(
                f"SELECT 1 FROM {COMPLETION_TABLE} WHERE product_id = ?",
            ...
            connection.execute(
                f"""
                INSERT INTO {PUBLICATION_EVENT_TABLE} (
                    product_id, event_type, wiki_path, content_sha256,
    ...
    def publication_status(self, product_id: Any) -> dict[str, Any]:
        completion = self.get_completion(product_id)
        ...
        if completion.published_at is None:
            mode = "pending"
        elif latest is not None and latest["event_type"] == "rerender_apply":
            mode = "rerender"
        else:
            mode = "direct"
        return {
            ...
            "mode": mode,
            "suppressed": self.is_suppressed(completion.product_id),
            "fence": self.publication_fence(completion.product_id),
        }
    ...
    def pending_publication(self) -> ResearchCompletion | None:
        ...
        for row in rows:
            ...
            if not self.is_suppressed(completion.product_id):
                return completion
        return None
        ...
        *,
        ...
        with self._state._write_transaction() as connection:
            ...
            if expected_fence is not None:
                ...
                if control is not None and control["action"] == "suppress":
                    raise PublicationFenceError(
                        "publication was suppressed before commit"
                    )
            if row["published_at"] is None:
                ...
                connection.execute(
                    ...
                    (
                        ...
                        path,
                        content_sha256_value,
                        _canonical_json({"wiki_action": action}),
                        _time_text(_utc(now)),
                    ),
                )
...
def publish_researched(
    ...
    *,
    ...
    if store.is_suppressed(completion.product_id):
        return {
            "ok": True,
            "found": True,
            "product_id": completion.product_id,
    ...
    expected_fence = store.publication_fence(completion.product_id)

    resolved_settings = settings or WikiSettings.from_env()
    render_model = RenderModel.from_completion(completion)
    product = {
        **render_model.product,
        "manufacturer": render_model.decision.get("manufacturer")
        or render_model.product.get("brand_code"),
        "model": render_model.decision.get("model")
        or render_model.product.get("product_name"),
    }
    path = stable_path(product, prefix=resolved_settings.path_prefix)
    title = " ".join(
        str(
            render_model.decision.get("model")
            or render_model.product.get("product_name")
    ...
    managed = render_product_page(
        product,
        render_model.decision,
        checked_at=completion.completed_at,
    )
    tags = _wiki_tags(render_model.product, render_model.decision)
    ...
    published = store.mark_published(
        ...
        content_sha256_value=content_sha256(managed),
        now=now,
    )
...
def rerender_completion(
    ...
    *,
    ...
    """Preview or explicitly apply a completion-only Wiki rerender."""

    completion = store.get_completion(product_id)
    if completion is None:
        raise ProductNotFoundError("completion does not exist")
    if not completion.publishable or completion.published_at is None:
        raise CompletionError(
            "rerender requires an already published completion"
        )
    if store.is_suppressed(product_id):
        raise CompletionError("rerender is suppressed by control state")
    expected_fence = store.publication_fence(product_id)
    render_model = RenderModel.from_completion(completion)
    resolved_settings = settings or WikiSettings.from_env()
    product = {
        **render_model.product,
    ...
    expected_path = stable_path(
        product,
        prefix=resolved_settings.path_prefix,
    )
    if not completion.wiki_path or completion.wiki_path != expected_path:
        raise CompletionError(
            "completion Wiki path differs from the stable path; move refused"
        )
    title = " ".join(
        str(
            render_model.decision.get("model")
    ...
    existing = client.get_page(
        completion.wiki_path,
        resolved_settings.locale,
    )
    if existing is None:
        raise CompletionError(
            "completion page is missing; create is refused"
        )
    if (
        existing.get("path") != completion.wiki_path
        or existing.get("locale") != resolved_settings.locale
    ):
        raise CompletionError("completion page identity changed; move refused")
    current_content = existing.get("content")
    if not isinstance(current_content, str):
        raise CompletionError("completion page content is invalid")
    merged = merge_auto_block(current_content, managed)
    changed = merged != current_content
    event_type = "rerender_apply" if apply else "rerender_preview"
    event_details: dict[str, Any] = {
        "changed": changed,
        "apply": apply,
    }
    action = "preview"
    page: Mapping[str, Any] = existing
    if apply:
        ...
        page_value = result.get("page")
        if isinstance(page_value, Mapping):
            page = page_value
        event_details["wiki_action"] = action
    new_fence = store.record_publication_event(
        completion.product_id,
        event_type=event_type,
        wiki_path=completion.wiki_path,
        content_sha256_value=content_sha256(merged),
    ...
    return {
        ...
        "fence": new_fence,
        "managed_content": managed if not apply else None,
    }
