...
class StateStore:
    ...
    def _migrate_postgresql(self) -> int:
        ...
        if current in {9, 10, 11}:
            with self._write_transaction() as connection:
                ...
                if locked_current not in {9, 10, 11}:
    ...
    def record_page_retirements(
        ...
        *,
        ...
        Every entry must identify the product's latest successful publication
        attempt and its stable Wiki path. Exact retries reuse the active audit
        rows; a conflicting retry fails the whole batch. Active leases are
        reclaimed when expired and otherwise rejected before any row is added.
        """
        ...
        with self._write_transaction() as connection:
            ...
            for entry in normalized_entries:
                ...
                if (
                    attempt_row["outcome"] != "synced"
                    or attempt_row["finished_at"] is None
                ):
                    raise StateError(
                        "page retirement source attempt is not a finished sync"
                    )
                ...
                if (
                    latest_row is None
                    or int(latest_row["attempt_id"]) != entry.source_attempt_id
                ):
                    raise StateError(
                        "page retirement must reference the latest synced attempt"
                    )
                try:
                    details = (
                        json.loads(attempt_row["details_json"])
                        if attempt_row["details_json"] is not None
                        else None
                    )
                except (TypeError, json.JSONDecodeError) as exc:
                    raise StateError(
                ...
                try:
                    audited_path = _page_retirement_path(details.get("wiki_path"))
                except (TypeError, ValueError) as exc:
                    raise StateError(
                ...
                if audited_path != entry.wiki_path:
                    raise StateError(
                        "page retirement Wiki path differs from publication audit"
                    )
                ...
                if active_row is None:
                    ...
                    exact = (
                        int(active_row["source_attempt_id"])
                        == entry.source_attempt_id
