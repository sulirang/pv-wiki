...
def _trusted_operator_group(
    requested_hostname: str,
    trusted_host_groups: Sequence[Sequence[str]],
) -> frozenset[str] | None:
    groups: list[frozenset[str]] = []
    for group_index, raw_group in enumerate(trusted_host_groups):
        if isinstance(raw_group, (str, bytes, bytearray)):
            raise ValueError(
                f"trusted_host_groups[{group_index}] must be a hostname array"
            )
        normalized = frozenset(
            str(host).strip().rstrip(".").casefold()
            for host in raw_group
            if isinstance(host, str) and host.strip()
        )
        if len(normalized) < 2:
            raise ValueError(
                "trusted operator groups must contain at least two hostnames"
            )
        if requested_hostname in normalized:
            groups.append(normalized)
    if not groups:
        return None
    if len(groups) != 1:
        raise PDFDownloadError(
            "PDF requested hostname belongs to ambiguous operator groups"
        )
    return groups[0]


def _redirect_hostname_allowed(
    requested_hostname: str,
    redirect_hostname: str,
    *,
    operator_group: frozenset[str] | None,
) -> bool:
    if redirect_hostname == requested_hostname:
        return True
    if operator_group is None:
        return False
    if (
        is_shared_source_hostname(requested_hostname)
        or is_shared_source_hostname(redirect_hostname)
    ):
        return False
    return redirect_hostname in operator_group
...
def download_pdf(
    ...
    *,
    ...
    requested_hostname = (
        urllib.parse.urlsplit(requested_url).hostname or ""
    ).rstrip(".").casefold()
    operator_group = _trusted_operator_group(
        requested_hostname,
        trusted_host_groups,
    )
    ...
    for redirect_number in range(MAX_REDIRECTS + 1):
        ...
        if not _redirect_hostname_allowed(
            requested_hostname,
            hostname,
            operator_group=operator_group,
        ):
            raise PDFDownloadError(
                "PDF redirect crossed the approved operator hostname boundary"
            )
        if current_url in redirect_chain:
            raise PDFDownloadError("PDF redirect chain contains a cycle")
...
def extract_pdf_evidence(
    ...
    *,
    ...
    if trusted_host_groups:
        download_arguments["trusted_host_groups"] = trusted_host_groups
    downloaded = downloader(url, **download_arguments)
    normalized_target_models = _normalized_target_models(target_models)
    ...
    return PDFEvidence(
        ...
        redirect_chain=(
            ...
            or tuple(
                dict.fromkeys(
                    (downloaded.requested_url, downloaded.final_url)
                )
            )
        ),
    )
