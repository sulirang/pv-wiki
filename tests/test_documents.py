...
class PDFDocumentTests(unittest.TestCase):
    ...
    def test_redirect_revalidates_dns_and_size_limit_is_enforced(self) -> None:
        ...
        with self.assertRaisesRegex(documents.PDFDownloadError, "non-public"):
            documents.download_pdf(
                ...
                timeout=5,
                trusted_host_groups=(
                    ("maker.example", "redirected.example"),
                ),
                resolver=resolver,
        ...
        with self.assertRaisesRegex(
            documents.PDFDownloadError,
            "operator hostname boundary",
        ):
            documents.download_pdf(
                "https://maker.example/model.pdf",
                max_bytes=1_000_000,
                ...
                connection_factory=lambda *_args: FakeConnection(
                    FakeResponse(
                        ...
                        headers={
                            "Location": (
                                "https://redirected.example/model.pdf"
                            )
                        },
                    )
                ),
    ...
    def test_shared_host_cannot_cross_tenants_even_in_operator_group(self) -> None:
        redirect = FakeConnection(
            FakeResponse(
                b"",
                status=302,
                headers={
                    "Location": "https://tenant-b.github.io/model.pdf",
                },
            )
        )
        with self.assertRaisesRegex(
            ...
            documents.download_pdf(
                "https://tenant-a.github.io/model.pdf",
                max_bytes=1_000_000,
                timeout=5,
                trusted_host_groups=(
                    ("tenant-a.github.io", "tenant-b.github.io"),
                ),
                resolver=public_resolver,
                connection_factory=lambda *_args: redirect,
            )
